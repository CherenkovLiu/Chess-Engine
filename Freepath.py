...
piececharacter = {'#':"",'P':"♙",'N':"♘",'B':"♙",'R':"♖",'Q':"♕",'K':"♔",'p':"♟",'n':"♞",'b':"♝",'r':"♜",'q':"♛",'k':"♚",' ':"╳"}
pst = {
    'P': [   0,   0,   0,   0,   0,   0,   0,   0,
...
#Openboard
def openboard():
    print(">Welcome to use the Chess-Engine 'Freepath'")
...
#This is a convert funtion that could convert two kinds of coordinate
def move_to_coordinate(makemove):
    return chr(96 + makemove[0] - (makemove[0]//10)*10) + str(makemove[0]//10) + chr(96 + makemove[1] - (makemove[1]//10)*10) + str(makemove[0]//10)

def coordinate_to_move(makemove):
    first = (9 - int(makemove[1])) * 10 + ord(makemove[0]) - 96
    second = (9 - int(makemove[3])) * 10 + ord(makemove[2]) - 96
    return [first,second]
    
#Use this function to print the board in a beautiful way
def printboard(board):
    listboard = list(board)
    for index,thing in enumerate(listboard):
        listboard[index] = piececharacter[thing]
